...
fn chip_hwcfg(is_j180: bool) -> Option<&'static HwConfig> {
...
pub unsafe extern "C" fn rust_gpu_initdata_size(
    ...
    let hwcfg = if let Some(h) = chip_hwcfg(false) {
        h
    } else {
        return -1;
    };
...
pub unsafe extern "C" fn rust_fill_gpu_initdata(
    ...
    let hwcfg = if let Some(h) = chip_hwcfg(is_j180 != 0) {
        h
    } else {
        return -1;
    };

    let mut perf_states = Vec::with_capacity(perf_state_count);
    for i in 0..perf_state_count {
        let mut volt_mv = Vec::with_capacity(perf_state_table_count);
        for j in 0..perf_state_table_count {
            volt_mv.push(unsafe {
                c_perf_states
                    .offset((i + j * perf_state_count) as isize)
                    .read()
                    .volt
            });
        }
        perf_states.push(PState {
            volt_mv,
            pwr_mw: unsafe { max_pwr.offset(i as isize).read() } / 1000,
            freq_hz: unsafe { c_perf_states.offset(i as isize).read().freq },
        });
    }

    let mut core_leak_coef = Vec::with_capacity(perf_state_table_count);
    let mut sram_leak_coef = Vec::with_capacity(perf_state_table_count);
    for i in 0..perf_state_table_count {
        core_leak_coef.push(unsafe { core_leak.offset(i as isize).read() });
        sram_leak_coef.push(unsafe { sram_leak.offset(i as isize).read() });
    }
    let csafr = if hwcfg.has_csafr {
        ...
        for i in 0..hwcfg.num_dies {
            leak_coef_cs.push(unsafe { cs_leak.offset(i as isize).read() });
            leak_coef_afr.push(unsafe { afr_leak.offset(i as isize).read() });
        }
        let mut perf_states_cs = Vec::with_capacity(n_perf_states_cs);
        let mut perf_states_afr = Vec::with_capacity(n_perf_states_afr);
        for i in 0..n_perf_states_cs {
            let mut volts = Vec::with_capacity(hwcfg.num_dies as usize);
            for j in 0..hwcfg.num_dies as usize {
                volts.push(unsafe {
                    pstates_cs
                        .offset((i + j * n_perf_states_cs) as isize)
                        .read()
                        .volt
                })
            }
            perf_states_cs.push(hw::PState {
                volt_mv: volts,
                pwr_mw: 0,
                freq_hz: unsafe { pstates_cs.offset(i as isize).read().freq },
            });
        }
        for i in 0..n_perf_states_afr {
            let mut volts = Vec::with_capacity(hwcfg.num_dies as usize);
            for j in 0..hwcfg.num_dies as usize {
                volts.push(unsafe {
                    pstates_afr
                        .offset((i + j * n_perf_states_afr) as isize)
                        .read()
                        .volt
                })
            }
            perf_states_afr.push(hw::PState {
                volt_mv: volts,
                pwr_mw: 0,
                freq_hz: unsafe { pstates_afr.offset(i as isize).read().freq },
            });
        }
