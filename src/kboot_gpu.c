...
int32_t rust_gpu_initdata_size(uint32_t compat_maj, uint32_t compat_min,
                               size_t *data_a, size_t *data_b, size_t *globals);

int32_t rust_fill_gpu_initdata(size_t perf_state_table_count, size_t perf_state_count,
                               const struct perf_state *c_perf_states, uint32_t *max_pwr, float *core_leak,
                               float *sram_leak, float *cs_leak, float *afr_leak, size_t n_perf_states_cs,
                               const struct aux_perf_state *pstates_cs, size_t n_perf_states_afr,
                               const struct aux_perf_state *pstates_afr, uint32_t is_j180,
                               uint32_t base_pstate, uint64_t uat_ttb_base,
                               uint32_t num_cores, uint32_t compat_maj,
                               uint32_t compat_min, uint32_t gpu_rev_id,
                               void *data_a, void *data_b, void *globals);
...
int dt_set_gpu(void *dt)
{
    ...
    if (rust_gpu_initdata_size(compat->num[0], compat->num[1], &data_a_size, &data_b_size, &globals_size) == -1)
        return -1;
    ...
    if (ADT_GETPROP(adt, sgx, "gpu-perf-base-pstate", &base_pstate) < 0)
        bail("ADT: GPU: missing gpu-perf-base-pstate\n");
    ...
    if (rust_fill_gpu_initdata(perf_state_table_count, perf_state_count, perf_states, max_pwr, core_leak, sram_leak,
                               cs_leak, afr_leak, n_perf_states_cs, pstate_cs_raw, n_perf_states_afr, pstate_afr_raw,
                               adt_is_compatible(adt, 0, "J180dAP"), base_pstate, uat_ttb_base, num_cores,
                               compat->num[0], compat->num[1], gpu_rev_id, data_a, data_b, globals) == -1)
        return -1;
